...
from franka_env.camera.hardware_depthai import DAICameraType, DepthAI
...
class FrankaEnv(gym.Env):
    ...
    def init_cameras(self, name_serial_dict=None):
        ...
        for cam_name, cam_specifications in name_serial_dict.items():
            if cam_specifications["type"] == "oak-d-lite" :
                cam_serial = cam_specifications["serial_number"]
                cap = VideoCapture(
                   DepthAI( device_id= cam_serial, name=cam_name, camera_type=DAICameraType.OAK_D_LITE), "oak-d"
                )
            elif cam_specifications["type"] == "oak-d-sr" :
                cam_serial = cam_specifications["serial_number"]
                cap = VideoCapture(
                   DepthAI( device_id= cam_serial, name=cam_name, camera_type=DAICameraType.OAK_D_SR), "oak-d"
                )
            else:raise NotImplementedError
            self.cap[cam_name] = cap
        print('Cameras initializes')
